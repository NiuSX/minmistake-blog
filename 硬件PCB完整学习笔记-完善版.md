...
用户需求
  ...
  -> 模块架构：电源、主控、传感、通信、驱动
  -> 电路实现：拓扑、器件、参数、保护、接口
  -> PCB 实现：层叠、布局、布线、回流、热、结构
  -> 制造实现：Gerber、BOM、CPL、钢网、装配
  -> 测试实现：测试点、固件、自检、治具、报告
  -> 产品结果：功能、可靠性、一致性、可维护性
```
...
```text
需求定义 -> 风险验证 -> 系统框图与电源树
-> 器件选型、计算和原理图 -> ERC 与设计评审
-> 层叠、规则、布局和布线 -> DRC/DFM/DFA/DFT
-> Gerber/BOM/CPL/装配资料 -> 打样与装配
-> 限流分步上电 -> 功能、边界、热和 EMC 验证
-> 根因分析、改版、回归 -> 版本发布与知识归档
```
...
```text
方案 A：12V -> Buck 5V -> LDO 3V3
方案 B：12V -> Buck 5V，12V -> 独立 Buck 3V3
方案 C：12V -> Buck 3V3，5V 使用独立升降压
```
...
```text
DE Enable → Driver Settling → UART Frame → Last Stop Bit → DE Disable
```
...
```text
外部能量源 → 连接器/外壳 → 一级泄放 → 限流/隔离 → 二级钳位 → 内部电路
```
...
```text
接口 → GDT/MOV/TVS → 电阻/PTC/电感 → 小信号钳位 → IC
```
...
```text
连接器 → 保险/限流 → 反接与反灌 → 浪涌钳位 → EMI 滤波 → 稳压器
```
...
```text
连接器 → 保护 → 滤波/隔离 → 收发器/ADC → MCU
```

输出链则反向：

```text
MCU → 驱动器 → 功率器件 → 保护/连接器 → 负载
```
...
```text
模拟连接器 → 保护 → 输入滤波 → 放大器 → ADC → 数字隔离/MCU
```
...
```text
电源输入 → VRM/LDO → PCB 电源与地
→ 去耦电容 → 封装 Ball/Lead/Substrate
→ 片上电源网格与片上电容 → 晶体管负载
```
...
```text
hardware-notes/
├─ fundamentals/       # 电路、器件、测量基础
├─ datasheets/         # 关键器件资料与勘误
├─ reference-designs/  # 官方参考设计
├─ calculations/       # 电源、热、阻抗、滤波计算
├─ footprints/         # 已验证封装与检查记录
├─ checklists/         # 评审、生产、上电清单
├─ failures/           # 故障现象、证据、根因、修复
└─ projects/           # 各项目设计与测试资料
```
